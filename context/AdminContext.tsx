...
interface AdminContextType {
  ...
  deleteItem: (
    table: "projects" | "experience" | "tech_stack",
    id: string,
  ) => Promise<void>;
    ...
    try {
      ...
      for (const draft of newProjects) {
        ...
        const { data: inserted, error } = await supabase
          .from("projects")
          .insert([
            { ...cleanData, display_order: 0, client_slug: CURRENT_SLUG },
          ])
          .select()
          .single();
        if (error) throw error;
      ...
      notify(
        "Error crítico: " + (error.message || "Fallo en la base de datos"),
        "error",
      );
    } finally {
      setIsSaving(false);

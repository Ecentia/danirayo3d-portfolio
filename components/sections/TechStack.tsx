...
const SubstanceDesignerIcon = ({ size, ...props }: any) => {
  const computedSize = size || "1.8em";
  return (
    <svg
      viewBox="0 0 512 512"
      fill="currentColor"
...
const MarvelousDesignerIcon = ({ size, className = "", ...props }: any) => {
  const computedSize = size || "1.8em";
  return (
    <div
    ...
    >
      <Image
        src="/marvelous-designer-logo.svg"
        alt="Marvelous Designer"
        fill
        className="object-contain grayscale brightness-125 opacity-90 group-hover:opacity-100 transition-all duration-500"
      />
      {/* Tinte morado al hacer hover, igual que el resto de iconos del panel */}
      <div className="absolute inset-0 bg-purple-400 mix-blend-color opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
    </div>
  );
};
const SubstancePainterIcon = ({ size, ...props }: any) => {
  const computedSize = size || "1.8em";
  return (
    <svg
      viewBox="0 0 512 512"
...
const ZbrushIcon = ({ size = 28, className = "", ...props }: any) => (
  ...
  >
    <Image
      ...
      alt="ZBrush"
      fill
      // opacity-[0.65] clava el tono gris (zinc-400) del resto de iconos
      // y group-hover:opacity-100 lo aclara al pasar el ratón o tocarlo
...
// --- ICONO EMBERGEN (logo real, mismo tratamiento que ZBrush/Marvelous) ---
const EmbergenIcon = ({ size, className = "", ...props }: any) => {
  const computedSize = size || "1.8em";
  return (
    <div
      className={`relative inline-block ${className}`}
    ...
    >
      <Image
        src="/embergen-seeklogo.svg"
        alt="Embergen"
        fill
        className="object-contain grayscale opacity-90 group-hover:opacity-100 transition-all duration-500 scale-125"
      />
      {/* Tinte morado al hacer hover, igual que el resto de iconos del panel */}
      <div className="absolute inset-0 bg-purple-400 mix-blend-color opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
    </div>
...
export const ICON_MAP: Record<string, React.FC<any>> = {
  // Custom
  SubstanceDesignerIcon,
  SubstancePainterIcon,
  MarvelousDesignerIcon,
  ZbrushIcon,
  EmbergenIcon,
...
export default function TechStack() {
  ...
  const getCategoryLabel = (id: string) => {
    if (id === "3D & TEXTURING")
      return isSpanish ? "3D y Texturizado" : "3D & Texturing";
    if (id === "POST & CREATIVE")
      return isSpanish ? "Postproducción y Creativo" : "Post & Creative";
    return id;
  };
  ...
  const handleSeedDB = async () => {
    const initialData = [
      ...
      },
      {
        ...
        category: "3D & TEXTURING",
        icon_key: "SubstanceDesignerIcon",
        display_order: 5,
      },
      {
        client_slug: CURRENT_SLUG,
        ...
        category: "3D & TEXTURING",
        icon_key: "MarvelousDesignerIcon",
        display_order: 6,
      },
      {
        client_slug: CURRENT_SLUG,
        name: "After Effects",
        category: "POST & CREATIVE",
        icon_key: "SiAdobeaftereffects",
        display_order: 7,
      },
      {
        client_slug: CURRENT_SLUG,
  ...
  return (
    <section
      className="relative w-full py-32 lg:py-48 bg-[#030305] overflow-hidden"
      id="tech-stack"
    >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-24 pb-8 border-b border-white/[0.05]">
          <div className="space-y-4">
            ...
            >
              <Monitor
                size={16}
                className="animate-pulse drop-shadow-[0_0_8px_rgba(220,38,38,0.8)]"
              />
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-red-500 to-red-800 font-bold">
                SOFTWARE_MODULES
              </span>
            </motion.div>
            ...
            <div className="flex flex-wrap gap-4 mt-6 md:mt-0">
              {techList.length === 0 && (
                <button
                  ...
                  className="flex items-center gap-2 bg-zinc-900 text-zinc-400 border border-zinc-700 px-5 py-3 rounded-full text-xs font-mono hover:bg-zinc-800 hover:text-white transition-all shadow-lg hover:border-zinc-500"
                >
                  <Database size={14} />{" "}
                  {isSpanish ? "CARGAR SISTEMA DEFECTO" : "LOAD DEFAULT SYS"}
                </button>
              )}
              <motion.button
              ...
              >
                ...
                />
                <span>
                  {isSpanish ? "AÑADIR SOFTWARE" : "ADD SOFTWARE NODE"}
                </span>
              </motion.button>
            </div>
          )}
                        ...
                        >
                          ...
                          {isAdmin && (
                            <button
                              ...
                              className="absolute top-2 right-2 bg-black/80 backdrop-blur border border-zinc-800 text-zinc-500 hover:text-red-500 hover:border-red-900 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all z-20 hover:scale-110"
                            >
                              <Trash2 size={14} />
                            </button>
                          )}
                          ...
                          <div className="w-14 h-14 rounded-xl bg-gradient-to-b from-zinc-800/50 to-black/50 border border-white/10 flex items-center justify-center shadow-inner group-hover:border-red-500/50 transition-all duration-500 relative z-10 group-hover:bg-red-950/20">
                            <IconComponent
                              size={28}
                              className="text-zinc-400 group-hover:text-white group-hover:drop-shadow-[0_0_12px_rgba(255,255,255,0.8)] transition-all duration-500 group-hover:scale-110"
                            />
                          </div>
          ...
          >
            ...
            >
              <button
                ...
                className="absolute top-6 right-6 text-zinc-500 hover:text-white transition-colors bg-zinc-900 p-2 rounded-full hover:bg-red-900/50"
              >
                <X size={18} />
              </button>
              ...
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-white font-black tracking-tighter text-2xl uppercase">
                  {isSpanish ? "Añadir Nodo" : "Insert Node"}
                </h3>
              </div>
              ...
              <form onSubmit={handleAddTech} className="flex flex-col gap-6">
                <div>
                  <label className="text-[10px] text-zinc-500 uppercase font-mono mb-2 block tracking-widest">
                    {isSpanish ? "Nombre_Nodo" : "Node_Name"}
                  </label>
                  <input
                    autoFocus
                    required
                    ...
                    className="w-full bg-[#0a0a0c] border border-zinc-800 rounded-xl p-4 text-sm text-white focus:border-red-500 focus:outline-none transition-colors font-mono focus:ring-1 focus:ring-red-500/50"
                    placeholder="e.g. Marvelous Designer"
                  />
                </div>
                <div>
                  <label className="text-[10px] text-zinc-500 uppercase font-mono mb-2 block tracking-widest">
                    {isSpanish ? "Categoría_Partición" : "Partition_Category"}
                  </label>
                  <select
                    ...
                    className="w-full bg-[#0a0a0c] border border-zinc-800 rounded-xl p-4 text-sm text-white focus:border-red-500 focus:outline-none transition-colors font-mono appearance-none focus:ring-1 focus:ring-red-500/50"
                  >
                    <option value="3D & TEXTURING">
                      {isSpanish ? "3D y Texturizado" : "3D & TEXTURING"}
                    </option>
                    <option value="POST & CREATIVE">
                      {isSpanish
                        ? "Postproducción y Creativo"
                        : "POST & CREATIVE"}
                    </option>
                  </select>
                </div>
                <div>
                  <label className="text-[10px] text-zinc-500 uppercase font-mono mb-2 block tracking-widest">
                    {isSpanish ? "Hash_Icono" : "Icon_Key_Hash"}
                  </label>
                  <select
                ...
                <button
                  type="submit"
                  className="mt-2 bg-gradient-to-r from-red-700 to-red-600 text-white font-bold tracking-widest py-4 rounded-xl hover:from-red-600 hover:to-red-500 transition-all active:scale-95 shadow-[0_0_30px_rgba(220,38,38,0.3)] uppercase"
                >

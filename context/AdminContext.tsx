...
interface AdminContextType {
  ...
  registerNewExperience: (data: any) => void; // <--- NUEVO
  saveAllChanges: () => Promise<void>;
  deleteItem: (table: 'projects' | 'experience', id: string) => Promise<void>; // <--- ACTUALIZADO
...
export function AdminProvider({ children }: { children: ReactNode }) {
  ...
  const [newExperience, setNewExperience] = useState<any[]>([]); // <--- NUEVO
  ...
  const saveAllChanges = async () => {
    if (Object.keys(pendingChanges).length === 0 && newProjects.length === 0 && newExperience.length === 0) return;
    setIsSaving(true);
    ...
    try {
      ...
      if (newExperience.length > 0) {
        // Limpiamos el ID temporal antes de enviar
        const cleanExperience = newExperience.map(({ id, ...rest }) => rest);
        const { error } = await supabase.from('experience').insert(cleanExperience);
        if (error) throw error;
      }

      // 3. Actualizaciones (Edición)
      const updatePromises = Object.entries(pendingChanges).map(async ([key, content]) => {
        if (key.startsWith('project_')) {
            await supabase.from('projects').update(content).eq('id', key.replace('project_', ''));
        } else if (key.startsWith('exp_')) {
            await supabase.from('experience').update(content).eq('id', key.replace('exp_', ''));
        } else {
            await supabase.from('portfolio_content').upsert({ client_slug: CURRENT_SLUG, section_id: key, ...content }, { onConflict: 'client_slug, section_id' });
        }
      });
      ...
      console.error(error);
      notify("Error critical: " + error.message, 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <AdminContext.Provider value={{ 
      ...
      saveAllChanges, deleteItem, notify, 

...
function MobileProjectDetail({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  const translatedTitle = getTranslation(project.title, isSpanish);
  const translatedDesc = getTranslation(project.description, isSpanish);
          ...
          >
            ...
            {loading ? (
              <div className="grid grid-cols-1 gap-6">
                {[1,2,3].map(i => (
                  <div key={i} className="w-full aspect-[4/3] bg-white/[0.02] border border-white/5 rounded-[2rem] animate-pulse" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-6">
                {projects.map((project, i) => {
                  const translatedCardTitle = getTranslation(project.title, isSpanish);
                  return (
                    <motion.div 
                      key={project.id}
